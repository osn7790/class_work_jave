...
public class MyArrayListTest {
    ...
    public static void main(String[] args) {
        ...
        // 4. 값 인덱스 번호로 지정해서 값 추가 하기
        list.add(1,1000);
        System.out.println("출력 확인 : " + list);
        ...
        // arrayInt.length; <- 변수
        System.out.println("사이즈 확인 : " + list.size()); // 메서드호출
        ...
        // 주소값을 복사 했다 ---> 얕은 복사 개념
        ArrayList nums = list;
        System.out.println("새로 선얺나 nums 에 값을 추가 했다.");
        nums.add(10);
        nums.add(500);
        ...
        System.out.println("리스트의 요소의 위치(index) 확인 : " + nums.indexOf(700));

        for(int i = 0; i < nums.size(); i++) {
            // nums[i] <--- 배열 방식
            System.out.println(nums.get(i)); // get 메서드를 통해서 접근 가능
